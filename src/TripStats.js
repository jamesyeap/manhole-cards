...
const uniqueWards = new Set(collectedCards.map((c) => c.ward)).size;
const uniqueCategories = new Set(collectedCards.map((c) => c.category)).size;

const categoryCounts = collectedCards.reduce((acc, c) => {
  ...
  return (
    <div className="trip-stats">
      <div className="trip-stat-item">
        <span className="trip-stat-number">{collectedCards.length}</span>
